...
<section class="container">
        <div class="row">
            ...
            <div class="col-10">
                ...
                <div class="ms-3">
                    <form role="form" action="${path }/board/update" method="post" id="writeForm" class="">
                    <input type="hidden" name="bNo" value="${b.bNo }">
	                    <div class="my-3" id="sDiv">
		                    <select class="form-select me-2 " onchange="changeSelect()" id="cat" name="bcNo" style="width:20%; display:inline-block">
		                        ...
		                        <c:if test="${mm.lNo <3 }">
		                        <option value="1" ${b.bcNo == "1" ? "selected" : "" }>공지사항</option>
		                        </c:if>
		                        <option value="2" ${b.bcNo == "2" ? "selected" : "" }>자유게시판</option>
		                        <option value="3" ${b.bcNo == "3" ? "selected" : "" }>사진게시판</option>
		                        <option value="4" ${b.bcNo == "4" ? "selected" : "" }>정모후기</option>
		                        <option value="5" ${b.bcNo == "5" ? "selected" : "" }>도전인증</option>
		                    </select>
	                	</div>
                        <div class="my-3">
                            <div class="col-12 mb-3">
                                <input type="text" class="form-control" name="bTitle" autocomplete="off" id="title" placeholder="제목" value="${b.bTitle }">
                            </div>
                            <textarea rows="" cols="" id="summernote" name="content">${b.bCont }</textarea>
                            <input type="hidden" id="first" name="fileName" value=""/>
                            <input type="hidden" id="first" name="oriName" value="${b.bRen }"/>
                            <div class="mt-2 offset-9-1">
                                ...
                                <button class="btn btn-outline-primary" type="button" style="padding: 9px 20px; font-weight:200;" onclick="location.href='${path}/board/detail?bNo=${b.bNo}'"> 취소 </button>
                            </div>
                        </div>
                    </form>
                </div>
	...
	<script>
		...
		function uploadSummernoteImageFile(file, el) {
      	...
      	function cancel(){
      		...
      		location.href="${path}/board?bcNo=${bcNo}";
