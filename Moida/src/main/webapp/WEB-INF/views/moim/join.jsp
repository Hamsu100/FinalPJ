    ...
    <section class=" container">
        <div class="row">
            ...
            <div class="col-10">
                ...
                <!-- 아코디언 -->
                <div class="accordion ms-2 mt-2 " id="joinlist">
                    <!-- 아코디언 아이템-->
                    <c:if test="${empty jmList }"><p style="width:100%;text-align:center">가입 신청서가 없습니다.</p></c:if>
                    <c:if test="${!empty jmList }">
                    <c:forEach var="jm" items="${jmList }" varStatus="status">
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="heading${jm.uNo }">
                            <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse${jm.uNo }" aria-expanded="false" aria-controls="collapse${jm.uNo }">
                                <img src="${path }/resources/upload/user/${jm.uRen}"  width="45" height="45" class="rounded-circle">
                                <span class = "ms-2" style="font-size: 130%"> ${jm.uNick } | ${jm.uAge } | ${jm.uGender } | ${jm.sido } ${jm.gugun } ${u.emd.equals(u.ri) ? "":u.ri }</span>
                            </button>
                        </h2>
                        <div id="collapse${jm.uNo }" class="accordion-collapse collapse" aria-labelledby="heading${jm.uNo }" data-bs-parent="#joinlist">
                            <div class="accordion-body">
                            	<c:if test="${m.gQ1 != null }">
                                <strong>질문 1 : ${m.gQ1 }</strong> <br> 답 : ${jm.jmA1 }<br><br>
                                </c:if>
                                <c:if test="${m.gQ2 != null }">
                                <strong>질문 2 : ${m.gQ2 } </strong> <br> 답 : ${jm.jmA2 }<br><br>
                                </c:if>
                                <c:if test="${m.gQ3 != null }">
                                <strong>질문 3 : ${m.gQ3 } </strong> <br> 답 : ${jm.jmA3 }<br>
                                </c:if>
                                <a class="btn btn-success btn-sm me-2" style=" margin-left: 87%" role="button" href="${path }/moim/accept?uNo=${jm.uNo}">승인</a>
                                <a class="btn btn-primary btn-sm " role="button" href="${path }/moim/deny?uNo=${jm.uNo}">거절</a>
                            </div>
                        </div>
                    </div>
                    </c:forEach>
